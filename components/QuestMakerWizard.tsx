        ...
        switch (refineStep) {
            ...
            case 'CARDS': return (<div><h3 className="text-lg font-bold mb-2">{t('chanceCards')}</h3>{draftQuest.chanceCards.map((card, index) => (
                <details key={index} className="bg-gray-800 rounded-lg mb-2">
                    ...
                    <div className="p-3 border-t border-gray-700 space-y-2">
                        <textarea value={getLocalizedString(card.description, language)} onChange={e => {const newCards = [...draftQuest.chanceCards];newCards[index].description = { ...newCards[index].description, [language]: e.target.value };handleDraftChange('chanceCards', newCards);}} className="w-full bg-gray-900 rounded-md p-1 h-16" />
                    </div>
                </details>
            ))}</div>);
            default: return null;
